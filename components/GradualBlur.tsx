...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      if (w <= 480 && (config as any)[`mobile${capKey}`]) v = (config as any)[`mobile${capKey}`]
      else if (w <= 768 && (config as any)[`tablet${capKey}`]) v = (config as any)[`tablet${capKey}`]
      else if (w <= 1024 && (config as any)[`desktop${capKey}`]) v = (config as any)[`desktop${capKey}`]
      setValue(v)
    }
...
type GradualBlurProps = Partial<GradualBlurConfig> & {
  preset?: keyof typeof PRESETS
  onAnimationComplete?: () => void
}
